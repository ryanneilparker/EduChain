...
web3.post('/createCertificate', async (req: Request, res: Response) => {
  try {
    ...
    if (receipt.status === 1) {
      ...
      // Extract the tokenId from the receipt's events

      const tokenId = receipt.tokenId.toNumber();
      res.status(200).json({ message: 'Certificate minted successfully', tokenId });
    } else {
      // Transaction failed
      res.status(500).json({ message: `Failed to create certificate: ${receipt}` });
    }
...
web3.get('/verifyCertificate', async (req: Request, res: Response) => {
  try {
    // Ensure you have the sender's private key (onlyOwner)
    const senderPrivateKey = '0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80'; // Replace with your private key
    ...
    res.status(500).json({ message: `Error verifiying certificate: ${error}` });
  }
});
